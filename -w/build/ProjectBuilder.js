...
var BuildSession, EventEmitter, Future, ProjectBuilder, ProjectWatcher, appConfig, buildManager, fileInfo, fs, fswalker, normalizePathSeparator, path, requirejs, requirejsConfig, rmrf, sourceModified, walkerFilter, _,
...
ProjectBuilder = (function(_super) {
  ...
  ProjectBuilder.prototype.build = function() {
    var appConfFile, appConfPromise, completePromise, corePromise, fullCompletePromise, nonWidgetFilesPromise, pathUtilsPromise, relativePos, scanBundle, scanCore, scanDir, scanRegularDir, showFinalMessage, start, widgetClassesPromise;
    ...
    showFinalMessage = function(verb) {
      var diff;
      diff = process.hrtime(start);
      return console.log("Build " + verb + " in " + (parseFloat((diff[0] * 1e9 + diff[1]) / 1e9).toFixed(3)) + " s");
    };
    return fullCompletePromise["catch"](function(err) {
